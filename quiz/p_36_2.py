...
best_score = 0.0
delta_tick = 0
# 점수는 좀 더 크게 보이도록 폰트 28로
fontScore = pygame.font.SysFont(None, 28)

def draw_score(surface, score_val: float, best_val: float):
    text = f"Score: {int(score_val)}   Best: {int(best_val)}"
    surf = fontScore.render(text, True, (255, 255, 255))
    rect = surf.get_rect(center=(width//2, 18))
    # 가독성용 그림자
    shadow = fontScore.render(text, True, (0, 0, 0))
    shadow_rect = shadow.get_rect(center=(width//2 + 1, 19))
    ...
    if current_box : 
        ...
        if missile_is_activate :
            ...
            if length > 0 :
                ...
                missile_direction = [missile_direction[0]/length , missile_direction[1]/length]
            else :
            ...
            score += 10 * (delta_tick / 1000)  # 점수 증가 속도 조절
            
            pygame.draw.circle(screen_surface,(255,0,0),(int(missile_pos[0]),int(missile_pos[1])),5)


    draw_score(screen_surface, score, best_score)
